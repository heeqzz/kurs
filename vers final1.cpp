...
void Euler2(int** graph, int v, int n, int* degree, int* result, int* index) {
    for (int u = 0; u < n; u++) {
        if (graph[v][u] == 1) {
            graph[v][u] = 0; // Удаляем ребро
            degree[v]--;     // Уменьшаем степень текущей вершины
            Euler2(graph, u, n, degree, result, index); // Рекурсивно продолжаем обход
        }
    }
    result[(*index)++] = v + 1; // Сохраняем вершину в результат
}
int main() {
    ...
    while (search != 0) {
        ...
        case 5: {
            if (graph == NULL) {
                printf("Ошибка: матрица не существует.\n");
                break;
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (degree[i] % 2 != 0) {
                    isEulerian = 0;
                    printf("Вершина %d имеет нечётную степень.\n", i + 1);
                    break;
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                if (degree[i] == 0) {
                    printf("Вершина %d имеет нулевую степень.\n", i + 1);
                    isEulerian = 0;
                    break;
                }
            }
